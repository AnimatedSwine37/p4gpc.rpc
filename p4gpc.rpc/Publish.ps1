...
param (
    ...
    ## => User Config <= ## 
    $ProjectPath = "p4gpc.rpc.csproj",
    $PackageName = "p4gpc.rpc",
    ...
    $GitHubRepoName = "p4gpc.rpc", # Name of the GitHub repo where the mod is contained
    ...
    $GameBananaItemId = 299970, # From mod page URL.

    $NuGetPackageId = "p4gpc.rpc",
...
function Publish-Common {
    ...
    $arguments = "$(Get-Common-Publish-Args -AllowDeltas $AllowDeltas) --outputfolder `"$Directory`" --publishtarget $PublishTarget --includeregexes fields\.json events\.json ModConfig\.json \.deps\.json \.runtimeconfig\.json"
